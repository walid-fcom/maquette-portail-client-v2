...
const ATTENDUS={
  'À étudier':'Proposition au client',
...
const CONTROLES=[
  {nom:'un besoin en recherche affiche des candidatures a etudier',fn:async page=>{
    await ouvrirCandidatures(page,'Consultant data gouvernance');
    const b=await badges(page);
    egal(b.length,2,'nombre de candidatures');
    egal(b.every(x=>x.libelle==='À étudier'),true,'libellés');
  ...
  {nom:'un desistement candidat se lit No Go Candidat',fn:async page=>{
    ...
    const b=await badges(page);
    egal(b.map(x=>x.libelle).sort().join(' | '),'No Go Candidat | À étudier','libellés');
